...
const app = {
  ...
  updateUI() {
    ...
    if (this.user) {
      ...
      // Навешиваем обработчики после изменения HTML
      ...
      if (createThreadBtn) {
        ...
        createThreadBtn.replaceWith(createThreadBtn.cloneNode(true));
        document.getElementById('createThreadBtn').addEventListener('click', () => this.showModal('createThreadModal'));
      }
      ...
      // Если пользователь не залогинен, также навешиваем обработчики
      const langBtn = document.getElementById('langBtn');
      const loginBtn = document.querySelector('[data-i18n="login"]');
      
      if (langBtn) {
        langBtn.replaceWith(langBtn.cloneNode(true)); // Удаляем старые обработчики
        document.getElementById('langBtn').addEventListener('click', () => this.toggleLang());
      }
      
      if (loginBtn) {
        loginBtn.replaceWith(loginBtn.cloneNode(true)); // Удаляем старые обработчики
        document.querySelector('[data-i18n="login"]').addEventListener('click', () => this.showModal('loginModal'));
      }
    }
    this.updateI18n();
  },
  ...
  renderPublic() {
    ...
    // Навешиваем обработчики клика на треды
  ...
  async viewThread(id) {
    try {
      ...
      // Навешиваем обработчик на форму ответа
      if (this.user) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Инициализация обработчиков для кнопок хедера
  const langBtn = document.getElementById('langBtn');
  ...
  if (loginBtn) {
    loginBtn.addEventListener('click', () => app.showModal('loginModal'));
  }
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', (e) => app.register(e));
  }
  
  if (createThreadForm) {
    createThreadForm.addEventListener('submit', (e) => app.createThread(e));
  }
  ...
  document.querySelectorAll('.nav-item').forEach(item => {
    ...
    if (section) {
      item.addEventListener('click', () => app.showSection(section));
    }
  });
  ...
  if (generateKeysBtn) {
    generateKeysBtn.addEventListener('click', () => app.generateKeys());
  }
  
  // Кнопка назад
  const backBtn = document.getElementById('backBtn');
  if (backBtn) {
    backBtn.addEventListener('click', () => app.backToList());
  }
