...
app.use(session({
  ...
  cookie: {
    ...
    sameSite: 'lax'  // Изменил с 'strict' на 'lax' для работы админки
  },
  proxy: true  // Добавил для работы за прокси (Railway)
}));
...
// Отдельный эндпоинт для получения replies
app.get('/api/threads/:id/replies', (req, res) => {
  try {
    const thread = db.prepare('SELECT is_private FROM threads WHERE id = ? AND is_deleted = 0').get(req.params.id);
...
// Pin/Unpin thread
app.post('/api/admin/threads/:id/pin', requireAdmin, (req, res) => {
  try {
    const thread = db.prepare('SELECT is_pinned FROM threads WHERE id = ?').get(req.params.id);
    if (!thread) {
      return res.status(404).json({ error: 'Thread not found' });
    }
    
    const newPinnedState = thread.is_pinned ? 0 : 1;
    db.prepare('UPDATE threads SET is_pinned = ? WHERE id = ?').run(newPinnedState, req.params.id);
    
    logSIEM('thread_pinned', 'low', req, { thread_id: req.params.id, pinned: Boolean(newPinnedState) });
    res.json({ success: true, is_pinned: Boolean(newPinnedState) });
  } catch (err) {
